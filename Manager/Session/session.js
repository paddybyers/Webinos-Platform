		...
		conn.on('data', function(data) {
			...
			if(parse.clientcert) {
				...
				data={'status':'',
				      ...
				      'servercert':fs.readFileSync(self.config.mastercertname).toString()};
				conn.write(JSON.stringify(data));
			}
		});
...
Client.prototype.connect = function(options, arg) {
	...
	client = tls.connect(port, arg, options, function(conn) {
...
Client.prototype.startServer = function() {
	...
	clientServer = tls.createServer(options, function(conn) {
		...
		conn.on('secure', function() {
			log('clientServer: connected secure : ' + conn.remoteAddress);
		});

		conn.on('data', function(data) {
			// Generate client certificate
			log('clientServer: read bytes = ' + data.length);
			var parse;
		...
		conn.on('end', function() {
			log('clientServer: end');
		});

		conn.on('close', function() {
			log('clientServer: socket closed');
			for(i = 0; i < self.connected_client.length; i += 1) {
				if(self.connected_client[i].commonname === self.config.common) {
					self.connected_client.pop(self.connected_client[i]);
					break;
		...
		conn.on('error', function(err) {
			log('clientServer:' + err + ' error stack : ' + err.stack);
		});
	});
	return clientServer;
};

exports.startAsServer = function(arg) {
	"use strict";
	var client = new Client();
	client.on('configread',function() {
		log('client read');
		var sock = client.startServer();
		sock.listen(port , arg);
	});
